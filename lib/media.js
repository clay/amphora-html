...
function getManifestScripts(state) {
  ...
  const componentsAndDependencies = state._components.reduce((acc, component) => {
    ...
    acc.components = _.union(acc.components, componentScripts);
  ...
  const scripts = _.union(runtime, globalScripts, componentsAndDependencies.dependencies, componentsAndDependencies.components, initScripts)
...
function injectScriptsAndStyles(state) {
  ...
  return function (html) {
    ...
    mediaMap.packs = injectTags(mediaMap.packs, locals.site, ASYNC_DEFER_SCRIPT_TAG);
