...
function getManifestScripts(state) {
  ...
  const scripts = ['__clay_globals', ...state._components, '__clay_client_init'].reduce((acc, component) => {
    const allDependencies = _.get(manifest, ['entrypoints', component, 'assets', 'js'], []),
      componentScripts = allDependencies.findAll(script => script.includes(component)),
      dependencyScripts = _.without(allDependencies, componentScript);

    componentScripts.forEach(acc.components.add);
    dependencyScripts.forEach(acc.dependencies.add);
    return acc;
  }, {
    components: new Set(),
  ...
  scripts.dependencies.delete(runtimeFile);
