...
function getManifestScripts(state) {
  ...
  const scripts = state._components.reduce((acc, component) => {
    const allDependencies = _.get(manifest, ['entrypoints', component, 'assets', 'js'], []),
      componentScript = allDependencies.find(script => script.includes(component)),
      dependencyScripts = _.without(allDependencies, componentScript);

    acc.components = acc.components.concat(componentScript);
    acc.dependencies = _.union(acc.dependencies, dependencyScripts);
    return acc;
  }, {
    components: [],
    dependencies: [],
  ...
  const runtimeFile = _.get(manifest, 'runtime.js'),
    initFiles = _.get(manifest, ['entrypoints', '__clay_client_init.js', 'assets', 'js']),
    dependenciesWithoutRuntime = _.without(scripts.dependencies, runtimeFile),
    combinedScripts = [
      runtimeFile,
  ...
  // return combinedScripts.map(script => path.join(assetHost, script));
  return initFiles.map(initFile => path.join(assetHost, initFile));
}
  ...
  return function (html) {
    ...
    return bluebird.props(mediaMap)
      .then(combinedFiles => {
        ...
        html = combinedFiles.packs ? appendMediaToBottom(combinedFiles.packs, html) : html; // If there are packs, append them
