  ...
  return function (html) {
    ...
    return bluebird.props(mediaMap)
      .then(combinedFiles => {
        ...
        if (combinedFiles.scripts && combinedFiles.scripts.length) {
          // hopefully can be parsed with downstream so we can graph
          const scriptObj = {
            size: combinedFiles.scripts.length
          };

          console.log('script_size', scriptObj);
        }
