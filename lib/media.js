 ...
 * each asset lists its complete set of dependencies, so we dedupe them, then
 * move the components' scripts to the end of the list and the Webpack runtime
 * to the beginning.
 *
 * @async
 * @param {Object} state - Amphora's render state.
 * @return {Promise<string[]>} A list of component assets.
 */
function getManifestScripts(state) {
  const site = state.locals.site,
    ...
    manifestFile = path.join(assetDir, manifestName);

  return files.readFilePromise(manifestFile)
    .then(rawManifest => JSON.parse(rawManifest))
    .then(manifest => {
      return bluebird.reduce(state._components, (acc, component) => {
        const allScripts = _.get(manifest, ['entrypoints', component, 'assets', 'js'], []),
          componentScript = scripts.find(script => script.includes(component)),
          dependencyScripts = _.without(allScripts, componentScript);

        acc.components = acc.components.concat(componentScript);
    ...
    .then(({ components, dependencies }) => {
      const runtimeFile = _.get(manifest, ['runtime.js']),
        dedupedDependencies = _.uniq(dependencies),
        dependenciesWithoutRuntime = _.without(dedupedDependencies, runtimeFile),
        dedupedScripts = _.uniq(components);

      return [
        runtimeFile,
        ...dependenciesWithoutRuntime,
        ...dedupedScripts,
      ];
    })
    .then(assets => assets.map(asset => path.join(assetHost, assetDir, asset)))
    .catch(() => []);
}
...
function injectScriptsAndStyles(state) {
  ...
  return function (html) {
    ...
    mediaMap.packs = injectTags(mediaMap.packs, locals.site, SCRIPT_TAG);
    ...
    return bluebird.props(mediaMap)
      .then(combinedFiles => {
        ...
        html = combinedFiles.packs ? appendMediaTo(combinedFiles.packs, html) : html; // If there are packs, append them
