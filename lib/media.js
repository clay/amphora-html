...
/**
 * Get the proper styles for the component. If a styleguide is configured, us
...
function getStyles(name, site) {
  const stylesSource = site.styleguide || site.slug,
    assetDir = site && site.assetDir || MEDIA_DIRECTORY,
    assetPath = site && site.assetPath || '',
    assetHost = site && site.assetHost || '',
    filenamesFilter = [
  ...
  if (!site.styleguide) {
    filenamesFilter.unshift(path.join(assetDir, 'css', name + '.css'));
  };

  return _.map(_.filter(filenamesFilter, files.fileExists), pathJoin(assetHost, assetPath, assetDir));
}
