...
function injectTags(fileArray, site, tag) {
  ...
  return bluebird.resolve(_.map(fileArray, function (file) {
    if (tag === STYLE_TAG) {
      ...
      return `<script defer type="text/javascript" src="${file}${buster}"></script>`;
    }
  }).join('\n'));
}
...
function getMediaMap(state) {
  return {
    packs: module.exports.getManifestScripts(state),
...
function getManifestScripts(state) {
  ...
  const scripts = state._components.reduce((acc, component) => {
  ...
  const runtimeFile = _.get(manifest, 'runtime.js');
  ...
  return function (html) {
    ...
    return bluebird.props(mediaMap)
      .then(combinedFiles => {
        ...
        return bluebird.props(mediaMap.packs).then(pack => {
          html = pack.runtime ? appendMediaToBottom(pack.runtime, html) : html;
          html = pack.global ? appendMediaToBottom(pack.global, html) : html;
          html = pack.components ? appendMediaToBottom(pack.components, html) : html;
          html = pack.init ? appendMediaToBottom(pack.init, html) : html;
          return html;
        });
      });
  };
