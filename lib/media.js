...
/**
 * [injectScriptsAndStyles description]
...
function injectScriptsAndStyles(state) {
  ...
  return function (html) {
    ...
    mediaProps = {}; // The object which we'll use in bluebird.props
    mediaMap = module.exports.getMediaMap(state);

      // allow site to change the media map before applying it
    if (setup.resolveMedia) mediaMap = setup.resolveMedia(mediaMap, locals);
    ...
    if (!locals.edit) {
      ...
      mediaProps.styles = module.exports.editStylesTags ? injectTags(mediaMap.styles, locals.site, STYLE_TAG) : combineFileContents(mediaMap.styles, 'public/css', '/css/', STYLE_TAG);
      mediaProps.scripts = module.exports.editScriptsTags ? injectTags(mediaMap.scripts, locals.site, SCRIPT_TAG) : combineFileContents(mediaMap.scripts, 'public/js', '/js/', SCRIPT_TAG);
    }
