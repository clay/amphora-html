...
function getMediaMap(state) {
  const scriptComponents = filterManifestComponents(state);
  const scriptState = Object.assign({}, state, { _components: scriptComponents });

  return {
    styles: module.exports.getStyleFiles(state),
    scripts: module.exports.getScriptFiles(scriptState),
...
function injectScriptsAndStyles(state) {
  ...
  return function (html) {
    ...
    // TODO: Ideally, we'd load these scripts asynchronously or after
    // the DOM is ready, setting the `async` or `defer` attribute.
    // Because the order in which they're parsed and executed currently
