...
function getMediaMap(state) {
  ...
  }, module.exports.getManifestScripts(state));
}
 ...
 */
function getManifestScripts(state) {
  const site = state.locals.site,
    assetDir = site && site.assetDir || MEDIA_DIRECTORY,
    assetHost = site && site.assetHost || '/',
    manifestName = site && site.manifestName || 'public/assets-manifest.json',
  ...
  const componentsAndDependencies = state._components.reduce((acc, component) => {
    const allDependencies = _.get(manifest, ['entrypoints', component, 'assets', 'js'], []),
      ...
      dependencyScripts = _.without(allDependencies, componentScripts);
  ...
  const scripts = {
    ...
    manifestGlobals: globalScripts,
    manifestInit: initScripts,
    manifestRuntime: [runtime],
  };

  const scriptsWithHost = _.mapValues(scripts, list => {
    const realList = list && list.filter(Boolean)

    if (!realList || !realList.length) return [];

    return Array.from(new Set(realList))
      .map(script => path.join(assetHost, script));
  });

  return scriptsWithHost;
}
...
function injectScriptsAndStyles(state) {
  ...
  return function (html) {
    ...
    if (!!mediaMap.packs && !!mediaMap.packs.runtime) {
      mediaMap.packs.runtime = injectTags(mediaMap.packs.runtime, locals.site, SCRIPT_TAG);
    }
    if (!!mediaMap.packs && !!mediaMap.packs.global) {
      mediaMap.packs.global = injectTags(mediaMap.packs.global, locals.site, ASYNC_SCRIPT_TAG);
    }

    if (!!mediaMap.packs && !!mediaMap.packs.components) {
      mediaMap.packs.components = injectTags(mediaMap.packs.components, locals.site, ASYNC_DEFER_SCRIPT_TAG);
    }

    if (!!mediaMap.packs && !!mediaMap.packs.init) {
      mediaMap.packs.init = injectTags(mediaMap.packs.init, locals.site, ASYNC_DEFER_SCRIPT_TAG);
    }
    ...
    return bluebird.props(mediaMap)
      .then(combinedFiles => {
        ...
        html = combinedFiles.manifestRuntime ? appendMediaToBottom(combinedFiles.manifestRuntime, html) : html; // If there are packs, append them
        html = combinedFiles.manifestGlobals ? appendMediaToBottom(combinedFiles.manifestGlobals, html) : html; // If there are packs, append them
        html = combinedFiles.manifestComponents ? appendMediaToBottom(combinedFiles.manifestComponents, html) : html; // If there are packs, append them
        html = combinedFiles.manifestInit ? appendMediaToBottom(combinedFiles.manifestInit, html) : html; // If there are packs, append them
...
// For testing
module.exports.getManifestScripts = getManifestScripts;
