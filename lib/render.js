...
function makeState(data, state) {
  ...
  return  _.assign(initialState, _.pick(state, ['_ref', '_layoutRef', 'locals']));
}
...
function setEnvVars(vars) {
  ...
  for (let i = 0; i < vars.length; i++) {
    ...
    ENV_VARS[`${vars[i]}`] = process.env[key];
  }
