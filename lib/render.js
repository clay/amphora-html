...
var { getIndices, getSchemaPath, getComponentPath, getLayoutPath, getLayoutName } = require('amphora-fs'),
...
function makeHtml(state) {
  return function (_data) {
    const template = state._layout ? clayUtils.getLayoutName(state._layout.uri) : clayUtils.getComponentName(state._self),
...
function makeState(data, meta) {
  ...
  if (meta._layoutRef) {
    ...
    initialState._layout = { name: layoutName, uri: meta._layoutRef };
  }
  ...
  if (meta.locals.edit) {
    ...
    if (layoutName) {
      initialState._layout.schema = getSchemaPath(getLayoutPath(layoutName));
    }
  }
