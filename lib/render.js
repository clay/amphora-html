...
function composeData(state) {
  const stateData = _.pick(state, ['_self', '_layoutRef', '_componentSchemas', '_envVars', 'locals']);

  stateData._componentVariations = styleguide.getVariations(state.locals.site.styleguide);

  return  _.assign(stateData, state._data);
}
