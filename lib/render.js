...
function composeData(state) {
  const componentVariations = styleguide.getVariations(state.locals.site.styleguide);

  _.assign(componentVariations, state._data._componentVariations);

  return  _.assign(_.pick(state, ['_self', '_layoutRef', '_componentSchemas', '_envVars', 'locals']), state._data);
}
