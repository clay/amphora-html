...
function makeHtml(state) {
  return function (_data) {
    ...
    // loop through the data looking for components that have variations but
    // do not have a default set
    _.forIn(_data, function (value, key) {
      styleguide.setDefaultVariation(_data[key]);
    });
...
function makeState(data, meta) {
  const componentVariations = styleguide.getVariations(meta.locals.site.styleguide);
