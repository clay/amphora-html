...
describe(_.startCase(filename), function () {
  let sandbox, fakeLog, getSchemaPathStub, getComponentPathStub, getIndicesStub, styleguideMock;
  ...
  beforeEach(function () {
    ...
    sandbox.spy(styleguide, 'getVariations');
    sandbox.spy(styleguide, 'setDefaultVariation');
  });
  ...
  describe('render', function () {
    ...
    it('attempt to set default component variations if a styleguide is set', function () {
      var data = mockData(),
        meta = mockPageMetaWithStyleguide(),
        res = mockRes(sandbox);
      ...
      return lib(data, meta, res)
        .then(() => {
          sinon.assert.called(styleguide.setDefaultVariation);
        });
    });
