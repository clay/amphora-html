...
describe(_.startCase(filename), () => {
  ...
  describe('render', () => {
    ...
    describe('makeState', () => {
      ...
      test('adds layout schema to _layout state in edit mode if given a _layoutRef', () => {
        const data = mockData(),
          meta = mockPageMeta();
        ...
        expect(fn(data, meta)._layout).toHaveProperty('schema');
      });
      ...
      test('does not add layout to state if not given a _layoutRef', () => {
        ...
        expect(fn(data, meta)).not.toHaveProperty('_layout');
      });
    });
  });
});
