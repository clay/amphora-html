...
// Placeholder for the package.json
var pkg;
...
function assignPkg(value) {
  pkg = value ? value : require(path.resolve(module.exports.rootPath, 'package.json'));
}
...
function getComponents() {
  ...
  return files.getFolders(path.resolve('components')).concat(npmComponents);
}
...
function getComponentPath(name) {
  ...
  if (getComponentName(name)) {
    modulePath = path.resolve(module.exports.rootPath, 'components', name);

    if (fs.existsSync(modulePath)) {
      result = modulePath;
    } else {
      npmName = getScopedModuleName(name);
      modulePath = npmName && path.resolve(module.exports.rootPath, 'node_modules', npmName);
      result = modulePath;
    }
  }
