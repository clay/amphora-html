 ...
 * @param {Object} payload
 */
function addCustomPartials(payload) {
  const partialsPath = 'handlebars-partials',
    resolvedPath = path.resolve(rootPath, partialsPath),
    existsDirectory = files.isDirectory(resolvedPath);
  var partialFiles;
  ...
  partialFiles = files.getFiles(resolvedPath);

  _.each(partialFiles, function (partialFile) {
    let templateFileContents = fs.readFileSync(path.resolve(resolvedPath, partialFile), 'utf8'),
      { name } = path.parse(partialFile);

    hbs.registerPartial(`_${name}`, templateFileContents);
  });
}
