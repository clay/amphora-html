...
describe(_.startCase(filename), () => {
  let fakeLog, getComponentPathStub, getComponentsStub, getLayoutsStub, getLayoutPathStub;

  beforeEach(() => {
    fakeLog = jest.fn();
    getComponentPathStub = jest.fn();
    getComponentsStub = jest.fn();
    getLayoutsStub = jest.fn();
    getLayoutPathStub = jest.fn();

    jest.spyOn(glob, 'sync');
    lib.setFsFns({ getComponentPathStub, getComponentsStub, getLayoutsStub, getLayoutPathStub });
  ...
  describe('init', () => {
    ...
    test('grabs handlebars templates from layout directories', () => {
      getLayoutsStub.mockReturnValue(['layout', 'one-column-layout']);
      getLayoutPathStub.mockImplementation((name) => {
        if (name === 'layout') return '/layouts/layout';
        if (name === 'one-column-layout') return '/layouts/one-column-layout';
        return undefined;
      });

      jest.spyOn(fs, 'readFileSync');
      jest.spyOn(nymagHbs, 'wrapPartial').mockImplementation((componentName) => {
        if (componentName === 'layout') return 'hello, world';
        return {};
      });
  ...
  describe('addPlugins', () => {
    ...
    test('skips plugins with no `render` or `postRender` function', () => {
      fn([{}]);
